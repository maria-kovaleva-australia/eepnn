...
#     os.makedir(f'{SAVE_PATH}/result', exist_ok=True)
...
def detect_PRs(e_norm_files, SAVE_PATH, fov, ant_start, ant_end, problematic_threshold, writer):
    for file_ in e_norm_files:
        if file_.endswith('enorm.mat'):
            print(f"Processing {file_}")
            freq, pol = F.match_freq_pol(file_)
            ...
            for antenna, pr_list in enumerate(PRs, start=ant_start):
                boxes = F.calculate_phi_theta_ranges(pr_list, PR_container[antenna - ant_start])
                lowest_dB_list = F.get_minimum_power_dB(pr_list, PR_container[antenna - ant_start])
                ant_max_power = e_norm[antenna - ant_start].max().max()

                p, t = divmod(e_norm[antenna - ant_start].values.argmax(), e_norm[antenna - ant_start].shape[1])
                p_adj = (p / 2) - 180
                t_adj = t / 2
                
                for box, (theta_range, phi_range) in enumerate(boxes):
                    minimum_dB_in_region = lowest_dB_list[box]
                    writer.writerow([problematic_threshold, theta_range, phi_range, antenna, freq, pol, fov, 
                                     minimum_dB_in_region, ant_max_power, (p_adj, t_adj)])
...
def main(fov, FEKO_SOURCE_PATH, SAVE_PATH, problematic_threshold, ant_start, ant_end):
    ...
    with open(output_path_csv, 'w', newline='') as f:
        ...
        e_norm_files = os.listdir(f"{SAVE_PATH}/e_norms")
        print("Detected files:", e_norm_files)
        detect_PRs(e_norm_files, SAVE_PATH, fov, ant_start, ant_end, problematic_threshold, writer)
    
    df = F.process_problematic_region_data(output_path_csv)
    df.to_excel(output_path_excel, index=False)
    
    print(f"Results saved to {SAVE_PATH}/result folder")
    print("Total running time:", (time.time() - start_time) / 60, "minutes")
